...
imageplot.bma <- function (bma.out, color = c("red", "blue", "#FFFFD5") , order = c("input", "probne0", "mds"), ...) 
{ 

    clr<- color
    if (length(color) == 1)
    {
        if (color == "default") clr<- c("#FF0000", "#FF0000", "#FFFFD5")
        if (color == "blackandwhite") clr<- c("black", "black", "white")
    }
    
    keep.mar<- par(mar = c(5,6,4,2) + .1)
  ...
  if ( ifelse(!is.null(bma.out$factor.type), bma.out$factor.type, FALSE) )
    {
        ...
        for (i in 1:length(assign))
        {
            ...
            which[,assign[[i]]-offset]<- bma.out$which[,i]
            
            nm<- names(bma.out$output.names)[i]
            if (!is.na(bma.out$output.names[[i]][1]))
                nm<- paste(nm, bma.out$output.names[[i]][-1], sep = ".")
    ...
    {
        postprob.rep<- matrix(bma.out$postprob, ncol=nvar, nrow=nmodel)
